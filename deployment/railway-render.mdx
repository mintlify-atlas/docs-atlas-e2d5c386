...
For both platforms, set the health check path to `/health`. This endpoint requires no authentication and returns `{"status":"ok"}` when the server is running correctly.
